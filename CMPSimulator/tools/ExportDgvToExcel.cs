...
namespace CMPSimulater.tools
{
    public class ExportDvgToExcel
    {
        ...
        public void OutputAsExcelFile(DataGridView dataGridView)
        {
            ...
            try
            {
                ...
                //range.Font.Size = 10;
                range.NumberFormat = "@";//设置数字文本格式,20190228晚上加的
                int r = 0;
                for (r = 0; r < rowNum; r++)
                {
                    for (int i = 0; i < columnNum; i++)
                    {
                        ...
                        //在obj.ToString()前加单引号是为了防止自动转化格式  ,以前是加的  '
                ...
                //保存工作已写入数据的工作表， Excel.XlFileFormat.xlExcel8 为解决整个问题的关键  20190228晚上修改
                // Excel.XlFileFormat.xlExcel8  对应  97-2003格式
                // Excel.XlFileFormat.xlExcel7  对应  5.0/95格式
                workbook.SaveAs(filePath, Excel.XlFileFormat.xlExcel8, Missing.Value, Missing.Value, Missing.Value, Missing.Value, Excel.XlSaveAsAccessMode.xlNoChange, Missing.Value, Missing.Value, Missing.Value, Missing.Value, Missing.Value);
            }
            catch (Exception ex)
            {
